        ...
        @foreach($destinations as $destination)
            <tr>
                ...
                <td width="15%">
                    ...
                    <a class="btn btn-danger btn-sm" href="#" onclick="confirm('are you sure?') ? $('#destination-delete-{{$destination->id}}').submit() : null ">
                        <span class="glyphicon glyphicon-trash"></span>
                        <form class="hidden" id="destination-delete-{{$destination->id}}" method="POST" action="{{ route('admin.destinations.destroy', $destination->id) }}">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                        </form>
                    </a>
